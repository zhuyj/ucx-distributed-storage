...
static int run_ucx_server(ucp_worker_h ucp_worker)
{
    ...
    printf("Receive iorequest from the client\n");
    ...
    printf("Receive Data from the client\n");
    recv_msg = malloc(info_tag.length);
    CHKERR_ACTION(recv_msg == NULL, "allocate memory\n", ret = -1; goto err);
    request = ucp_tag_msg_recv_nb(ucp_worker, recv_msg, info_tag.length,
                                  ucp_dt_make_contig(1), msg_tag, recv_handler);
    ...
    printf("Send ioresponse to the client\n");
